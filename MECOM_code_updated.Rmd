...
grch37_counts <- read_tsv("/Users/tejsharm/Desktop/MECOM_Project/Mecom_paper/MECOM_paper_files/rsem.merged.gene_counts_re2_RT68.tsv")
...
# Import samplesheet
sampleIDs <- read.xlsx("Sample_Sheet_2rp.xlsx") 

# Build dGE list, Create sample Dataframe
targets <- data.frame(manifestID = colnames(counts[2:21])) %>% 
  left_join(sampleIDs, by = c("manifestID" = "Manifest_ID")) 

# Build DGEList object for differential expression analysis
dge_list_grch37 <- DGEList(counts = as.matrix(counts_annotated[2:21]), 
...
keep <- rowSums(edgeR::cpm(dge_list_grch37)>1) >= 2
...
# Filtering top 10 significant genes for each contrast id's
top <- 10 
...
genes_of_interest_C42B_ER <- c("PBX1", "MAPK11", "FOXA1", "NOTCH1", "GATA2", "AR", "MBNL2")
...
#  Combine with specific genes of interest
genes_of_interest_CWRR1 <- c("PBX1", "MAPK11", "FOXA1", "NOTCH1", "GATA2", "AR", "MBNL2", "RARA", "ADAM12")
...
CWRR1_shMECOM <- ggplot(data %>% dplyr::filter(contrast_id %in% c("CWRR1_shMECOM_v_control")), aes(logFC, -log10(PValue))) +
  ...
  annotate("text", x = 4.5, y = min(-log10(data$PValue)) + 4, 
            label = sprintf("Up-regulated:     %6d\nDown-regulated:%6d\nTotal genes:         %6d\np-value:               %6.2f\nq-value:               %6.2f\nlogFC:                  %6.2f", 
                         round(upregulated_genes, 2), 
                         round(downregulated_genes, 2), 
                         total_genes, 
...
gsea_Annotated <- gsea_Annotated %>% left_join(msigdb %>% distinct(gs_name,gs_collection, gs_subcollection), by = c("ID"="gs_name"))
